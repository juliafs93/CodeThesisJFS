function[] = mhmm_interference_fun(FolderPath,project,trace_struct_final,bin_groups,Tres,w,K,inference_times,t_window,n_bootstrap)
        ...
        for b = 1:n_bootstrap % iterate through bootstrap replicates
            ...
            if skip_flag
                ...
                parfor i_local = 1:n_localEM % Parallel Local EM                
                    ...
                    PP = cellfun(@(x) char(join(string(round(exp(x(2,:)))),'')),local_out.soft_struct.p_z_log_soft,'UniformOutput',false);
                    Ptransitions = cellfun(@(x) length(strfind(x,'01')) + length(strfind(x,'10')),PP);
                    local_struct(i_local).pp_transitions = sum(Ptransitions);
                end 
                
                [logL, max_index] = max([local_struct.logL]); % Get index of best result   
                %[logL, max_index] = min([local_struct.pp_transitions]); % minimize promoter transitions                    
