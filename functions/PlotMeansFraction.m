function[Fig] = PlotMeans(MaxF,MeanNorm, OnOff,TimeScale,Selected,Properties,Bits,ColorArg,Fig,OnlyMeans,Legend, XLim, YLimits)
    ...
    %try; 
        ...
        ylim(YLimits); xlabel('time into nc14 (min)'); ylabel('Mean Fluorescence (AU)')
        ...
        errorbar(TimeScale,nanmean(MaxFSelected,2),nanstd(MaxFSelected,1,2)./sqrt(sum(~isnan(MaxFSelected),2)),...
            '- .','MarkerSize',8,'Color',[ColorArg(6,:),0.5],'DisplayName',[Legend,' n = ',num2str(sum(Selected))],'CapSize',0,'LineWidth',0.25);
    ...
    %try;
        ...
        Mean = nanmean(MaxFSelected(:,Midline),2);
        SEM = nanstd(MaxFSelected(:,Midline),1,2)./sqrt(sum(~isnan(MaxFSelected(:,Midline)),2));
        ...
        %plot(TimeScale,Mean,'-','MarkerSize',8,'Color',[ColorArg(6,:),0.5],'DisplayName',[Legend,' n = ',num2str(sum(Midline)),' cells, ',num2str(size(MeanNorm,2)),' embryos'],'LineWidth',0.5);
        %Poly = polyshape([TimeScale,flip(TimeScale)]',[Mean+SEM;flip(Mean-SEM)]);
    ...
    %try;
        ...
        %plot(TimeScale,nanmean(MaxFSelected(:,Midline),2),'- .','MarkerSize',12,...
        %    'Color',ColorArg(6,:),'MarkerEdgeColor',ColorArg(6,:),'DisplayName',Legend); 
        errorbar(TimeScale,nanmean(MeanNorm,2),nanstd(MeanNorm,1,2)./sqrt(sum(~isnan(MeanNorm),2)),...
            '- .','MarkerSize',8,'Color',[ColorArg(6,:),0.5],'DisplayName',[Legend,' n = ',num2str(size(MeanNorm,2))],'CapSize',0,'LineWidth',0.25);
        legend boxoff
        legend('Location','NorthWest')
...
%             'Color',[ColorArg(6,:),0.5],'DisplayName',Legend,'LineWidth',0.5);
        plot(TimeScale,[nansum(OnOffSelected(:,Midline),2)./nansum(~isnan(OnOffSelected(:,Midline)),2)],...
            '- .','MarkerSize',8,...
             'Color',ColorArg(6,:),'MarkerEdgeColor',ColorArg(6,:),'DisplayName',[Legend,' n = ',num2str(sum(Midline))]); 
        legend boxoff
        legend('Location','NorthWest')
         %end
%     try; subplot(414); hold on
