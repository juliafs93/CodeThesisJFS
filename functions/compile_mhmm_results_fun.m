function[glb_all,hmm_results] = compile_mhmm_results_fun(FolderPath,project,w,Tres,alpha,K,t_window)
% Compile results into summary structure. Generate summary plots
% addpath('~/Google Drive jf565/MATLAB_R_scripts/functions/utilities/');
% close all
% clear 
%------------------------------Set System Params--------------------------%
...
% Tres = 15; %Time Resolution
% alpha = 1.5; % MS2 rise time in time steps
% alpha = 1.8; % MS2 rise time in time steps
% t_window = 15*60;
% t_window = 30*60;

%-----------------------------ID Variables--------------------------------%
% id variables
% project = 'test_1boots'; %project identifier
...
OutPath = [FolderPath project '/' id_string];
mkdir(OutPath)

%%% path to inference results
folder_path = [FolderPath project '/' id_string '/out/'];
...
for i = 1:length(files)
    if ~isempty(strfind(files(i).name,['w' num2str(w)])) && ...
       ...
       ~isempty(strfind(files(i).name,'.mat'))
...
for i = 1:length(glb_all)    
    ...
    %Obtain raw R matrix
    R = prob_to_rate(A,Tres); %rates s-1, Kon 2,1 Koff 1,2
    glb_all(i).R_mat = R;
    Rcol = reshape(R,1,[]); %rates s-1, Kon 2 Koff 3
    glb_all(i).R = Rcol;
    ...
    elseif false %sum(Rcol<0)>K
        ...
        glb_all(i).R_fit = 60*glb_all(i).R_mat; % now fit into min-1
    ...
    for t = 1:length(time_index)
        ...
        if K == 2 % added JF
            k_on_rat = R_fit_array(bin_vec==bin&time_vec==time,2)./...
                            R_fit_array(bin_vec==bin&time_vec==time,3);
            med_on_ratio(t,b) = median(k_on_rat);
            std_on_ratio(t,b) = .5*(quantile(k_on_rat,.75,1)-...
                                   quantile(k_on_rat,.25,1));
            k_off_rat = R_fit_array(bin_vec==bin&time_vec==time,4)./...
                            R_fit_array(bin_vec==bin&time_vec==time,1);
...
for i = 1:length(bin_range)
    ...
    %             std_pi0(k,i,t) = std(pi0_all(k,bin_vec==bin&time_vec==time));
        ...
        %added to stop dimensions error
        med_eff_time(i,t) = NaN;
    end
end
% off rate, on rate and emission rates 
if K == 2 % added 19/08/18 JF
    ...
    for b = 1:length(bin_range)
        ...
        for t = 1:length(time_index)
            time = time_index(t);        
            %Calculate and Store R moments  
            r01_vec = R_fit_array(bin_vec==bin&time_vec==time,2);
            r10_vec = R_fit_array(bin_vec==bin&time_vec==time,3);
            pi0_vec = occupancy(1,bin_vec==bin&time_vec==time)';
            pi1_vec = occupancy(2,bin_vec==bin&time_vec==time)';
            on_rate_all = r01_vec;
            on_rate_eff_med(t,b) = median(on_rate_all);
            on_rate_eff_ste(t,b) = .5*(quantile(on_rate_all,.75)-quantile(on_rate_all,.25));
            off_rate_all = r10_vec;
            off_rate_eff_med(t,b) = median(off_rate_all);
            off_rate_eff_ste(t,b) = .5*(quantile(off_rate_all,.75)-quantile(off_rate_all,.25));
            init_rate_all = pi1_vec.*initiation_rates(2,bin_vec==bin&time_vec==time)';  
...
save([OutPath '/glb_all_tw' num2str(round(t_window/60)) '_K' num2str(K) '.mat'],'glb_all')
